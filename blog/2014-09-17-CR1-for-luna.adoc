...
Ever wondered why Eclipse couldn't just restart your deployed module(s) and you could continue working without have to wait for a restart of
...
the dialog above shows up giving you the choice wether you want to restart the modules, terminate the server, restart it or just continue as nothing happened.

But beyond this it also allow you to say "Remember this choice for this server", allowing you to e.g. click "Restart modules" once and then all consequent hot code replace
failures will just automatically trigger a restart of just the modules.
...
To support latest Hibernate 4.3 and JPA 2.1 we had to "rewire" large part of Hibernate Tools internals.
In past Hibernate 3.6 was hardwired to be used for loading users mapping configurations for the UI - that is no longer the case.
The configured version for your console configuration is now used everywhere instead of only for codegeneration.

I'll spare you the details here but just outline that the rewiring has been completed and we now support
...
Using these is good if you find our automatic detection is too eager or too weak in finding that you need the
